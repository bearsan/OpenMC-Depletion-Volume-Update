...
class Results:
    ...
    def update_volumes(self, geometry):
        """Updates volumes from depletion results to geometry (in Multi-physics coupling)
        ...
        for cell in geometry.get_all_material_cells().values():
            for material in cell.get_all_materials().values():
                if material.depletable:
                    self.volume[str(material.id)] = material.volume